...
function clearBoard() {
    if (window.confirm('Bạn có chắc chắn muốn xóa bảng?')) {
...
function handleGetUserMediaError(e) {
    switch (e.name) {
        case "NotFoundError":
            alert("Không thể mở cuộc gọi của bạn vì không tìm thấy máy ảnh hoặc micrô.");
            break;
        case "SecurityError":
        case "PermissionDeniedError":
            break;
        default:
            alert("Lỗi khi mở máy ảnh hoặc micrô của bạn: " + e.message);
            break;
    }
...
function screenShareToggle() {
    ...
    screenMediaPromise
        ...
        .catch((e) => {
            alert("Không thể chia sẻ màn hình:" + e.message);
            console.error(e);
        });
}
